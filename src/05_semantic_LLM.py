...
def main():
    ...
    for conference in CONFERENCES:
        ...
        for paper in [
            ...
            # skip paper if too many intro references are missing
            if len(missing_intro) > 3:
                print("Missing too many intro references")
                continue

            # remove generated references that are not in the intro
            ...
            if os.path.exists(
                ...
                os.path.join(
                    conference_dir,
                    paper,
                    f"{STRATEGY}_refs_of_refs_{ENGINE}.json",
                )
            ):
                print("file exists")
                gen_refs = pd.read_csv(
            ...
            gen_refs["In Paper"] = None
            gen_refs["In Intro"] = None
            gen_refs["Not Found"] = None
    ...
    merged_gen_refs.to_csv(
        os.path.join(results_dir, f"merged_gen_refs_{STRATEGY}_{ENGINE}.csv"),
        index=False,
    )

    summary_statistics.to_csv(
        os.path.join(
            results_dir, f"summary_statistics_{STRATEGY}_{ENGINE}.csv"
        ), index=False
    )

    with open(
        os.path.join(
            results_dir, f"merged_gen_refs_of_refs_{STRATEGY}_{ENGINE}.json"
        ),
        "w",
    ) as json_file:
    ...
    merged_intro_refs.to_csv(
        os.path.join(results_dir, f"merged_intro_refs_{STRATEGY}_{ENGINE}.csv"),
        index=False,
    )
    ...
    with open(os.path.join(results_dir, f"merged_intro_refs_of_refs_{STRATEGY}_{ENGINE}.json"), "w") as json_file:
